...
os.environ.setdefault("SQLITE_DB_PATH", str(project_root / "data" / "transactions.db"))
...
def test_post_batch_happy_path(client):
    ...
    payload = [
        ...
        # Duplicate within batch to test in-batch deduplication
        {
            "transaction_id": tx_id2,
            "timestamp": "2026-05-19T10:05:00",
            "user_id": 42,
    ...
    assert data["inserted_records"] == 2  # 3 records received, 1 duplicate in batch skipped
...
# 12. POST /transactions/batch Database Idempotency / Deduplication (No Silent Overwrite)
def test_post_batch_idempotency_database(client):
    ...
    assert resp1.json()["ignored_records"] == 0
    
    # Second batch post with identical transaction_id (should be ignored and NOT overwrite)
    payload2 = [{
        "transaction_id": tx_id,
        "timestamp": "2026-05-19T12:00:00",
        "user_id": 45,
        "merchant_id": 300,
        "amount": 250.00,  # Different amount
        "category": "Food",
        "country_code": "MX",
        "status": "completed"
    }]
    ...
    assert tx_id in resp2.json()["ignored_ids"]
    
    # Query user transactions to verify the initial amount (100.00) remains, NO silent overwrite occurred
    tx_resp = client.get("/users/45/transactions?page=1&page_size=100")
    assert tx_resp.status_code == 200
    txs = tx_resp.json()["transactions"]
    # Find the inserted transaction to verify the initial amount (100.00) remains (no overwrite occurred)
